...
public class expertvagabond {
	private static String urlBase = "https://expertvagabond.com/travel-blog/";
	...
	private static void parseHtml(String blogUrl, Path outputFile, FSDataOutputStream mapbw, FileSystem fs) {
		try {
	        ...
	        Elements divlist = doc.select("div[class=entry-content]");
	        String buf = "";
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path file = new Path(metaBaseDir + "expertvagabond-mapping.json");
			if (fs.exists(file))
				fs.delete(file);
			...
			for(String blogUrl : urlList) {
				cnt ++ ;
				Path blogOutput = new Path(blogBaseDir + "expertvagabond/blog_" + cnt + ".txt");
