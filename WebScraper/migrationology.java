...
public class migrationology {
	private static String urlBase = "http://www.migrationology.com/blog/";
	private static int totalPages = 55;
	...
	private static void parseHtml(String blogUrl, Path outputFile, FSDataOutputStream mapbw, FileSystem fs) {
		try {
	        ...
	        Elements divlist = doc.select("article[id=content] > p");
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 1; i <= totalPages; i ++ ) {
		        ...
		        String responseString = EntityUtils.toString(entity, "UTF-8");
				Document doc = Jsoup.parse(responseString);
				Elements links = doc.select("div[class=thumbnail] > a"); // a with title
				links.forEach(e -> {
			...
			Path file = new Path(metaBaseDir + "migrationology-mapping.json");
			if (fs.exists(file))
				fs.delete(file);
			...
			for(String blogUrl : urlList) {
				cnt ++ ;
				Path blogOutput = new Path(blogBaseDir + "migrationology/blog_" + cnt + ".txt");
