...
public class ytravelblog {
	private static String urlBase = "http://www.ytravelblog.com/blog/";
	private static int totalPages = 119;
	...
	private static void parseHtml(String blogUrl, Path outputFile, FSDataOutputStream mapbw, FileSystem fs) {
		try {
	        ...
	        Elements styleNodes = doc.select("style ~ *");
	        Elements plist = doc.select("section[class=content eight columns post-single] > p");
	        if (plist.size() == 0)
	        	plist = doc.select("section[class=\"content eight columns post-single disclosure\"] > p");
	        String buf = "";
	        for(Element e:plist) {
	        	String text = "";
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 1; i <= totalPages; i ++ ) {
		        ...
		        String responseString = EntityUtils.toString(entity, "UTF-8");
				Document doc = Jsoup.parse(responseString);
				Elements links = doc.select("a[class=read-more]"); // a with title
				links.forEach(e -> {
			...
			Path file = new Path(metaBaseDir + "ytravelblog-mapping.json");
			if (fs.exists(file))
				fs.delete(file);
			...
			for(String blogUrl : urlList) {
				cnt ++ ;
				Path blogOutput = new Path(blogBaseDir + "ytravelblog/blog_" + cnt + ".txt");
