...
public class nomadicmatt {
	...
	private static void parseHtml(String blogUrl, Path outputFile, FSDataOutputStream mapbw, FileSystem fs) {
		try {
	        ...
	        Elements divlist = doc.select("main[class=content]");
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path file = new Path(metaBaseDir + "nomadicmatt-mapping.json");
			if (fs.exists(file))
				fs.delete(file);
			...
			for(String blogUrl : urlList) {
				cnt ++ ;
				Path blogOutput = new Path(blogBaseDir + "nomadicmatt/blog_" + cnt + ".txt");
