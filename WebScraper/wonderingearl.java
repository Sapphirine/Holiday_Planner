...
public class wonderingearl {
	private static String urlBase = "http://www.wanderingearl.com/blog/";
	...
	private static void parseHtml(String blogUrl, Path outputFile, FSDataOutputStream mapbw, FileSystem fs) {
		try {
	        ...
	        Elements plist = doc.select("div[class=\"entry-content content\"]");
	...
	public static void main(String[] args) {
		...
		try {
			...
			Path file = new Path(metaBaseDir + "wonderingearl-mapping.json");
			if (fs.exists(file))
				fs.delete(file);
			...
			for(String blogUrl : urlList) {
				cnt ++ ;
				Path blogOutput = new Path(blogBaseDir + "wonderingearl/blog_" + cnt + ".txt");
